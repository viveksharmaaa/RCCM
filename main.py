...
parser.add_argument('--lambda', type=float, dest='_lambda', default=1.5, help='Convergence rate: lambda')
...
# Optimization variable, alpha(Tube size) initialization. Here, mu is a free variable
mu = torch.nn.Linear(1,1,bias=False)
alpha = torch.nn.Linear(1,1,bias=False)
alpha.weight = torch.nn.Parameter(torch.tensor([8.0]))
mu.weight = torch.nn.Parameter(torch.tensor([8.0]))
...
def forward(x, xref, uref, _lambda, verbose=False, acc=False, detach=False):
    ...
    K = Jacobian(u, x)
    w = torch.rand(bs,num_dim_distb).unsqueeze(-1)
    A = DfDx + sum([u[:, i, 0].unsqueeze(-1).unsqueeze(-1) * DBDx[:, :, :, i] for i in range(num_dim_control)]) + sum([torch.rand(1) * DBwDx[:, :, :, i] for i in range(num_dim_distb)])
    ...
    loss = 0
    loss += loss_pos_matrix_eigen_values(- Cond1  - epsilon * torch.eye(Cond1.shape[-1]).unsqueeze(0).type(x.type()))
    loss += loss_pos_matrix_eigen_values(Cond2)
    loss += alpha(torch.ones(1))
    loss += loss_pos_matrix_random_sampling(W - args.w_lb * torch.eye(W.shape[-1]).unsqueeze(0).type(x.type()))
    loss += loss_pos_matrix_eigen_values(-C1_LHS_1 - epsilon * torch.eye(C1_LHS_1.shape[-1]).unsqueeze(0).type(x.type()))
